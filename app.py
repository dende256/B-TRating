...
def view_result(result_id):
    ...
    filepath = os.path.join(app.config['RESULTS_FOLDER'], f'{result_id}.json')
    ...
    try:
        with open(filepath, 'r') as f:
            data = json.load(f)
        return render_template('result_view.html', result=data, result_id=result_id)
...
def upload_csv():
    ...
    try:
        ...
        # Get optional title from form and save result
        ...
        if save_result:
            ...
            result_data = {
                ...
                'win_probabilities': win_probs_rounded,
                'charts': charts,
                'matches': [[w, l] for w, l in matches[:100]]  # Save first 100 matches
            }
            ...
            response_data['result_url'] = url_for('view_result', result_id=result_id, _external=True)
