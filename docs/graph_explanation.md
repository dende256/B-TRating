...
逐次反映するレーティング算出アルゴリズムと異なり、全試合結果をまとめて計算させるのが特徴です。
これにより逐次反映型に存在する順序依存問題（試合する順序がレーティングに影響を及ぼす）を避けることができます。
...
## 最終レーティング
- 平均レーティングでソートしランキング化したものです。
- 各プレイヤー名をクリックすると、そのプレイヤー視点で他のプレイヤーへの推定勝率が表示されます。
- これにより、特定のプレイヤーが他のプレイヤーに対してどれくらい勝てるかを直感的に把握できます。

## グラフ解説


### 1. レーティンググラフ（log-strength）
...
- 横軸のプレイヤーが、縦軸のプレイヤーに対する勝率を示しています。
- 色が濃いほど勝率（緑色）／負け率（赤色）が高いことを示します。
